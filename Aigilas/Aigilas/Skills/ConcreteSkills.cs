...
namespace Aigilas.Skills
{
    ...
    {
        public NoSkill()
            : base(SkillId.NO_SKILL, AnimationType.NONE)
        {
    ...
    public class AcidDripSkill : ISkill
    {
        public AcidDripSkill()
            : base(SkillId.ACID_DRIP, AnimationType.STATIONARY)
        {
            StartOffCenter = true;
            AddCost(StatType.MANA, 10);
            Add(Elements.WATER);
    ...
    public class AcidNozzleSkill : ISkill
    {
        public AcidNozzleSkill()
            : base(SkillId.ACID_NOZZLE, AnimationType.STATIONARY)
        {
            AddCost(StatType.MANA, 10);
 ...
 ii++)
    ...
    {
        public DartSkill()
            : base(SkillId.DART, AnimationType.RANGED)
        {
            AddCost(StatType.MANA, 10);
            Add(Elements.DARK);
    ...
    public class DartTrapSkill : ISkill
    {
        public DartTrapSkill()
            : base(SkillId.DART_TRAP, AnimationType.RANGED)
        {
            AddCost(StatType.MANA, 10);
            Add(Elements.DARK);
 ...
 ii++)
    ...
    public class FloorSpikesSkill : ISkill
    {
        public FloorSpikesSkill()
            : base(SkillId.FLOOR_SPIKES, AnimationType.STATIONARY, float.MaxValue, true)
        {
 ...
 ii++)
    ...
    public class RemoteMineSkill : ISkill
    {
        ...
        public RemoteMineSkill()
            : base(SkillId.REMOTE_MINE, AnimationType.STATIONARY)
        {
            AddCost(StatType.MANA, 10);
            Add(Elements.FIRE);
 ...
 ii++)
            ...
            for (int ii = 1;
 ii < GameManager.TileMapHeight - 1;
 ii++)
            {
    ...
    public class VaporImplantSkill : ISkill
    {
        public VaporImplantSkill()
            : base(SkillId.VAPOR_IMPLANT, AnimationType.RANGED)
        {
            AddCost(StatType.MANA, 10);
    ...
    public class WallPunchSkill : ISkill
    {
        ...
        public override void Affect(IEntity target)
        {
            ...
            {
                if (target.GetLocation().GridX > 0 && target.GetLocation().GridX < Dungeons.DungeonFactory.BlocksWide - 1 && target.GetLocation().GridY > 0 && target.GetLocation().GridY < Dungeons.DungeonFactory.BlocksHigh - 1)
                {
                    target.SetInactive();
